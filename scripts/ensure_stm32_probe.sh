...
# Prints the selector to stdout and caches it in .stm32-probe.
# Resolution order:
# 1) PROBE env (if connected)
# 2) PORT env alias (if connected)
# 3) cached .stm32-probe (if connected)
# 4) single ST-Link present (VID:PID 0483:3748)
