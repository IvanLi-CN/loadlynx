...
#[cfg(stm32g4)]
fn g4_hpre_divisor() -> u32 {
    match pac::RCC.cfgr().read().hpre() {
        pac::rcc::vals::Hpre::DIV1 => 1,
        pac::rcc::vals::Hpre::DIV2 => 2,
        pac::rcc::vals::Hpre::DIV4 => 4,
        pac::rcc::vals::Hpre::DIV8 => 8,
        pac::rcc::vals::Hpre::DIV16 => 16,
        pac::rcc::vals::Hpre::DIV64 => 64,
        pac::rcc::vals::Hpre::DIV128 => 128,
        pac::rcc::vals::Hpre::DIV256 => 256,
        pac::rcc::vals::Hpre::DIV512 => 512,
        _ => 1,
    }
}

#[cfg(stm32g4)]
fn g4_hclk2_frequency() -> Hertz {
    let freqs = unsafe { rcc::get_freqs() };
    g4_sys_frequency()
        .map(|sys| Hertz(sys.0 / g4_hpre_divisor()))
        .or_else(|| freqs.hclk2.to_hertz())
        .expect("DAC kernel clock HCLK2 is not running")
}
    ...
    pub async fn write(&mut self, data: ValueArray<'_>, circular: bool) {
        ...
        let tx_f = match data {
            ValueArray::Bit8(buf) => unsafe {
                dma.write(buf, T::regs().dhr8r(C::IDX).as_ptr() as *mut u8, tx_options)
            },
...
impl<'d, T: Instance, C: Channel, M: PeriMode> DacChannel<'d, T, C, M> {
    ...
    fn set_hfsel(&mut self) {
        #[cfg(stm32g4)]
        let frequency = g4_hclk2_frequency();
        #[cfg(not(stm32g4))]
        let frequency = T::frequency();

        if frequency >= crate::time::mhz(160) {
            critical_section::with(|_| {
                T::regs().mcr().modify(|reg| {
                    reg.set_hfsel(0b10);
                });
            });
        } else if frequency >= crate::time::mhz(80) {
            critical_section::with(|_| {
                T::regs().mcr().modify(|reg| {
                    reg.set_hfsel(0b01);
                });
