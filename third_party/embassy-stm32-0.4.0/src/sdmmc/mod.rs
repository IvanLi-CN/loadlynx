...
use sdio_host::sd::{
    BusWidth, CardCapacity, CardStatus, CurrentState, SDStatus, CIC, CID, CSD, OCR, RCA, SCR, SD,
};
...
impl<'d, T: Instance> Sdmmc<'d, T> {
                ...
                while timeout > 0 {
                    let ready_for_data = match card {
                        SdmmcPeripheral::Emmc(_) => {
                            self.read_status::<EMMC>(card)?.ready_for_data()
                        }
                        SdmmcPeripheral::SdCard(_) => {
                            self.read_status::<SD>(card)?.ready_for_data()
                        }
                    };
        ...
        match card {
            SdmmcPeripheral::SdCard(ref mut card) => {
                ...
                let ocr = loop {
                    ...
                    // Initialize card
                    match Self::cmd(
                        sd_cmd::sd_send_op_cond(true, false, true, voltage_window),
                        false,
                    ) {
                        // ACMD41
                        Ok(_) => (),
            ...
            SdmmcPeripheral::SdCard(_) => {
                ...
                if freq.0 > 25_000_000 {
                    ...
                    if self.signalling == Signalling::SDR25 {
                        ...
                        if self.read_status::<SD>(self.card.as_ref().unwrap())?.state()
    ...
    pub async fn init_sd_card(&mut self, freq: Hertz) -> Result<(), Error> {
        self.init_internal(freq, SdmmcPeripheral::SdCard(Card::default()))
            .await
    }
    ...
    pub async fn init_emmc(&mut self, freq: Hertz) -> Result<(), Error> {
        self.init_internal(freq, SdmmcPeripheral::Emmc(Emmc::default()))
            .await
    }
...
impl<'d, T: Instance> block_device_driver::BlockDevice<512> for Sdmmc<'d, T> {
    ...
    async fn read(
        ...
        if buf.len() == 1 {
            ...
            let blocks: &mut [DataBlock] = unsafe {
                core::slice::from_raw_parts_mut(buf.as_mut_ptr() as *mut DataBlock, buf.len())
            };
