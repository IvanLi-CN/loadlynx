...
impl<'d, T: Instance> Cordic<'d, T> {
    ...
    pub fn blocking_calc_32bit(
        ...
        self.peri.set_argument_count(if arg1_only {
            AccessCount::One
        } else {
            AccessCount::Two
        });

        self.peri.set_result_count(if res1_only {
            AccessCount::One
        } else {
            AccessCount::Two
        });
        ...
        impl<'d, T: Instance> Cordic<'d, T> {
            ...
            pub fn $func_arg1_name(&self, arg: $float_type) -> Result<(), ArgError> {
                ...
                let err_info = match config.function {
                    Cos | Sin | Phase | Modulus | Arctan if !(-1.0..=1.0).contains(arg) => {
                        Some(Arg1ErrInfo {
                            scale: None,
                            range: [-1.0, 1.0],
                    ...
                    Ln => match config.scale {
                        ...
                        Scale::Arg1o16Res16 if !(0.4375..0.584).contains(arg) => {
                            Some(Arg1ErrInfo {
            ...
            pub fn $func_arg2_name(&self, arg: $float_type) -> Result<(), ArgError> {
                ...
                let err_info = match config.function {
                    Cos | Sin if !(0.0..=1.0).contains(arg) => {
                        Some(Arg2ErrInfo { range: [0.0, 1.0] })
                    }

                    Phase | Modulus if !(-1.0..=1.0).contains(arg) => {
                        Some(Arg2ErrInfo { range: [-1.0, 1.0] })
                    }

                    Cos | Sin | Phase | Modulus | Arctan | Cosh | Sinh | Arctanh | Ln | Sqrt => {
                        None
                    }
                };
