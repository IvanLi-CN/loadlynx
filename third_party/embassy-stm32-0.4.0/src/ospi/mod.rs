...
impl<'d, T: Instance, M: PeriMode> Ospi<'d, T, M> {
    ...
    fn new_inner(
        ...
        #[cfg(octospim_v1)]
        {
            ...
            crate::pac::RCC
                .ahb2smenr()
                .modify(|w| w.set_octospimsmen(true));
            #[cfg(stm32u5)]
            crate::pac::RCC
                .ahb2enr1()
                .modify(|w| w.set_octospimen(true));
    ...
    fn configure_command(
        ...
        if <enums::OspiWidth as Into<u8>>::into(command.iwidth)
            > <enums::OspiWidth as Into<u8>>::into(self.width)
            || <enums::OspiWidth as Into<u8>>::into(command.adwidth)
                > <enums::OspiWidth as Into<u8>>::into(self.width)
            || <enums::OspiWidth as Into<u8>>::into(command.abwidth)
                > <enums::OspiWidth as Into<u8>>::into(self.width)
            || <enums::OspiWidth as Into<u8>>::into(command.dwidth)
                > <enums::OspiWidth as Into<u8>>::into(self.width)
        {
    ...
    pub fn blocking_read<W: Word>(
        ...
        if T::REGS.ccr().read().admode() == vals::PhaseMode::NONE {
            T::REGS
                .ir()
                .write(|v| v.set_instruction(current_instruction));
        } else {
...
pub trait Instance:
    SealedInstance + PeripheralType + RccPeripheral + SealedOctospimInstance
{
}
