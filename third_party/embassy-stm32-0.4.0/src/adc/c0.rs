...
impl<'d, T: Instance> Adc<'d, T> {
    ...
    pub fn enable_temperature(&self) -> Temperature {
        debug!(
            "Ensure that sample time is set to more than temperature sensor T_start from the datasheet!"
        );
        T::common_regs().ccr().modify(|reg| {
            reg.set_tsen(true);
        });
    ...
    fn setup_channel_sequencer<'a>(
        channel_sequence: impl ExactSizeIterator<Item = &'a mut AnyAdcChannel<T>>,
    ) {
        assert!(
