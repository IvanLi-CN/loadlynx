...
#[cfg(stm32g4)]
fn g4_pll_input_frequency() -> Option<Hertz> {
    let pllcfgr = pac::RCC.pllcfgr().read();
    match pllcfgr.pllsrc() {
        pac::rcc::vals::Pllsrc::HSI => Some(rcc::HSI_FREQ),
        pac::rcc::vals::Pllsrc::HSE => unsafe { rcc::get_freqs() }.hse.to_hertz(),
        _ => None,
    }
}

#[cfg(stm32g4)]
fn g4_pll_vco_frequency() -> Option<Hertz> {
    let pllcfgr = pac::RCC.pllcfgr().read();
    let src = g4_pll_input_frequency()?;
    let prediv = u32::from(pllcfgr.pllm().to_bits()) + 1;
    let mul = u32::from(pllcfgr.plln().to_bits());
    Some(Hertz(src.0 / prediv * mul))
}

#[cfg(stm32g4)]
fn g4_pll1_p_frequency() -> Option<Hertz> {
    let pllcfgr = pac::RCC.pllcfgr().read();
    if !pllcfgr.pllpen() {
        return None;
    }

    let vco = g4_pll_vco_frequency()?;
    Some(Hertz(vco.0 / u32::from(pllcfgr.pllp().to_bits())))
}

#[cfg(stm32g4)]
fn g4_pll1_r_frequency() -> Option<Hertz> {
    let pllcfgr = pac::RCC.pllcfgr().read();
    if !pllcfgr.pllren() {
        return None;
    }
    ...
    let vco = g4_pll_vco_frequency()?;
    let div = 2 * (u32::from(pllcfgr.pllr().to_bits()) + 1);
    Some(Hertz(vco.0 / div))
}

#[cfg(stm32g4)]
fn g4_sys_frequency() -> Option<Hertz> {
    match pac::RCC.cfgr().read().sws() {
        pac::rcc::vals::Sw::HSI => Some(rcc::HSI_FREQ),
        pac::rcc::vals::Sw::HSE => unsafe { rcc::get_freqs() }.hse.to_hertz(),
        pac::rcc::vals::Sw::PLL1_R => g4_pll1_r_frequency(),
        _ => None,
    }
}

#[cfg(stm32g4)]
fn g4_adc_kernel_frequency() -> Hertz {
    let freqs = unsafe { rcc::get_freqs() };
    match pac::RCC.ccipr().read().adc12sel() {
        pac::rcc::vals::Adcsel::PLL1_P => g4_pll1_p_frequency()
            .or_else(|| freqs.pll1_p.to_hertz())
            .expect("ADC kernel clock PLL1_P is not running"),
        pac::rcc::vals::Adcsel::SYS => g4_sys_frequency()
            .or_else(|| freqs.sys.to_hertz())
            .expect("ADC kernel clock SYS is not running"),
        _ => panic!("ADC kernel clock mux is not configured"),
    }
}
...
impl<'d, T: Instance> Adc<'d, T> {
    ...
    pub fn new(adc: Peri<'d, T>) -> Self {
        ...
        #[cfg(stm32g4)]
        let ker_ck = g4_adc_kernel_frequency();
        #[cfg(not(stm32g4))]
        let ker_ck = T::frequency();

        let prescaler = Prescaler::from_ker_ck(ker_ck);

        T::common_regs()
            .ccr()
            .modify(|w| w.set_presc(prescaler.presc()));

        let frequency = Hertz(ker_ck.0 / prescaler.divisor());
    ...
    #[cfg(stm32g4)]
    pub fn enable_regular_oversampling_mode(
        &mut self,
        mode: Rovsm,
        trig_mode: Trovs,
        enable: bool,
    ...
    pub async fn read(
        ...
        assert!(
            sequence.len() != 0,
            "Asynchronous read sequence cannot be empty"
        );
        assert!(
