...
impl<'d, T: Instance> DsiHost<'d, T> {
    ...
    pub fn new(_peri: Peri<'d, T>, te: Peri<'d, impl TePin<T>>) -> Self {
        ...
        te.set_as_af(
            te.af_num(),
            AfType::output(OutputType::PushPull, Speed::Low),
        );
        /*
                T::regs().wcr().modify(|w| {
                    w.set_dsien(true);
    ...
    pub fn read(
        ...
        match packet_type {
            ...
            GenShortPktReadP1(param1) => {
                self.config_packet_header(channel_id, packet_type, param1, 0)
            }
            GenShortPktReadP2(param1, param2) => {
                self.config_packet_header(channel_id, packet_type, param1, param2)
            }
