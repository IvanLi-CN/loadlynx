    ...
    async fn write_dma_internal(
        ...
        poll_fn(|cx| {
            ...
            if remaining_len == total_len {
                ...
                if let Err(e) =
                    Self::reload(self.info, remaining_len.min(255), !last_piece, timeout)
                {
...
impl<'d> I2c<'d, Async, MultiMaster> {
    ...
    pub async fn respond_to_write(&mut self, buffer: &mut [u8]) -> Result<usize, Error> {
        let timeout = self.timeout();
        timeout
            .with(self.read_dma_internal_slave(buffer, timeout))
            .await
    }
    ...
    pub async fn respond_to_read(&mut self, write: &[u8]) -> Result<SendStatus, Error> {
        let timeout = self.timeout();
        timeout
            .with(self.write_dma_internal_slave(write, timeout))
            .await
    }
    ...
    //
    // returns the total number of bytes received
    async fn read_dma_internal_slave(
        &mut self,
        buffer: &mut [u8],
        timeout: Timeout,
    ) -> Result<usize, Error> {
        ...
        let total_received = poll_fn(|cx| {
            ...
            if remaining_len == total_len {
                ...
                if let Err(e) =
                    Self::reload(self.info, remaining_len.min(255), !is_last_slice, timeout)
                {
                    return Poll::Ready(Err(e));
                }
                remaining_len = remaining_len.saturating_sub(255);
    ...
    async fn write_dma_internal_slave(
        &mut self,
        buffer: &[u8],
        timeout: Timeout,
