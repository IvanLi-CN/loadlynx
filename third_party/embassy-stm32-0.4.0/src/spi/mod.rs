...
impl<'d> Spi<'d, Blocking> {
    ...
    pub fn new_blocking<T: Instance>(
        ...
        Self::new_inner(
            ...
            new_pin!(
                mosi,
                AfType::output(OutputType::PushPull, config.gpio_speed)
            ),
    ...
    pub async fn write<W: Word>(&mut self, data: &[W]) -> Result<(), Error> {
        ...
        let tx_f = unsafe {
            self.tx_dma
                .as_mut()
                .unwrap()
                .write(data, tx_dst, Default::default())
        };
        ...
        let i2scfg = regs.i2scfgr().modify(|w| {
            w.i2smod().then(|| {
                ...
                w.set_i2scfg(match prev {
                    vals::I2scfg::SLAVE_RX | vals::I2scfg::SLAVE_FULL_DUPLEX => {
                        vals::I2scfg::SLAVE_RX
                    }
                    vals::I2scfg::MASTER_RX | vals::I2scfg::MASTER_FULL_DUPLEX => {
                        vals::I2scfg::MASTER_RX
                    }
    ...
    pub async fn read<W: Word>(&mut self, data: &mut [W]) -> Result<(), Error> {
        ...
        let rx_f = unsafe {
            self.rx_dma
                .as_mut()
                .unwrap()
                .read(rx_src, data, Default::default())
        };
