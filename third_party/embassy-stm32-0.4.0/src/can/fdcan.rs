...
impl<'d> CanConfigurator<'d> {
    ...
    pub fn start(self, mode: OperatingMode) -> Can<'d> {
        let ns_per_timer_tick =
            calc_ns_per_timer_tick(&self.info, self.periph_clock, self.config.frame_transmit);
...
impl<'c, 'd, const TX_BUF_SIZE: usize, const RX_BUF_SIZE: usize>
    BufferedCan<'d, TX_BUF_SIZE, RX_BUF_SIZE>
{
    fn new(
        info: &'static Info,
...
pub type RxFdBuf<const BUF_SIZE: usize> =
    Channel<CriticalSectionRawMutex, Result<FdEnvelope, BusError>, BUF_SIZE>;
...
impl RxMode {
    ...
    fn try_read_fd<T: Instance>(
        &self,
        ns_per_timer_tick: u64,
    ) -> Option<Result<FdEnvelope, BusError>> {
        if let Some((frame, ts)) = T::registers().read(0) {
            let ts = T::registers().calc_timestamp(ns_per_timer_tick, ts);
