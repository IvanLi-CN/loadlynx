...
use embassy_usb_synopsys_otg::{
    on_interrupt as on_interrupt_impl, Bus as OtgBus, ControlPipe, Driver as OtgDriver, Endpoint,
    In, OtgInstance, Out, PhyType, State,
};
    ...
    fn init(&mut self) {
        ...
        if T::HIGH_SPEED {
            critical_section::with(|_| {
                ...
                rcc.ahb1lpenr()
                    .modify(|w| w.set_usb_otg_hsulpilpen(_ulpien));
            });
        }
        ...
        critical_section::with(|_| {
            ...
            if T::HIGH_SPEED {
                ...
                rcc.ahb1lpenr()
                    .modify(|w| w.set_usb_otg_hs_ulpilpen(_ulpien));
            } else {
                rcc.ahb1enr().modify(|w| w.set_usb_otg_fs_ulpien(_ulpien));
                rcc.ahb1lpenr()
                    .modify(|w| w.set_usb_otg_fs_ulpilpen(_ulpien));
            }
        });
