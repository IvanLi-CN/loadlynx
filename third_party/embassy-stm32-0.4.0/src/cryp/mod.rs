...
impl<'c, const KEY_SIZE: usize> Cipher<'c> for AesGcm<'c, KEY_SIZE> {
    ...
    async fn post_final<T: Instance>(
        ...
        if dir == Direction::Encrypt {
            ...
            let read = Cryp::<T, Async>::read_bytes(
                cryp.outdma.as_mut().unwrap(),
                Self::BLOCK_SIZE,
                &mut out_data,
            );
            let write = Cryp::<T, Async>::write_bytes(
                cryp.indma.as_mut().unwrap(),
                Self::BLOCK_SIZE,
                int_data,
            );
...
#[cfg(any(cryp_v2, cryp_v3, cryp_v4))]
impl<'c, const KEY_SIZE: usize, const TAG_SIZE: usize, const IV_SIZE: usize>
    AesCcm<'c, KEY_SIZE, TAG_SIZE, IV_SIZE>
{
    ...
    for AesCcm<'c, KEY_SIZE, TAG_SIZE, IV_SIZE>
{
    ...
    async fn init_phase<T: Instance>(&self, p: pac::cryp::Cryp, cryp: &mut Cryp<'_, T, Async>) {
        ...
        Cryp::<T, Async>::write_bytes(cryp.indma.as_mut().unwrap(), Self::BLOCK_SIZE, &self.block0)
            .await;
    ...
    async fn post_final<T: Instance>(
        ...
        if dir == Direction::Decrypt {
            ...
            Cryp::<T, Async>::write_words(cryp.indma.as_mut().unwrap(), Self::BLOCK_SIZE, &in_data)
                .await;
        }
    }
}
...
#[cfg(any(cryp_v2, cryp_v3, cryp_v4))]
impl<'c, const TAG_SIZE: usize, const IV_SIZE: usize> CipherSized
    for AesCcm<'c, { 128 / 8 }, TAG_SIZE, IV_SIZE>
{
}
#[cfg(any(cryp_v2, cryp_v3, cryp_v4))]
impl<'c, const TAG_SIZE: usize, const IV_SIZE: usize> CipherSized
    for AesCcm<'c, { 192 / 8 }, TAG_SIZE, IV_SIZE>
{
}
#[cfg(any(cryp_v2, cryp_v3, cryp_v4))]
impl<'c, const TAG_SIZE: usize, const IV_SIZE: usize> CipherSized
    for AesCcm<'c, { 256 / 8 }, TAG_SIZE, IV_SIZE>
{
}
#[cfg(any(cryp_v2, cryp_v3, cryp_v4))]
impl<'c, const KEY_SIZE: usize, const IV_SIZE: usize> CipherAuthenticated<4>
    for AesCcm<'c, KEY_SIZE, 4, IV_SIZE>
{
}
#[cfg(any(cryp_v2, cryp_v3, cryp_v4))]
impl<'c, const KEY_SIZE: usize, const IV_SIZE: usize> CipherAuthenticated<6>
    for AesCcm<'c, KEY_SIZE, 6, IV_SIZE>
{
}
#[cfg(any(cryp_v2, cryp_v3, cryp_v4))]
impl<'c, const KEY_SIZE: usize, const IV_SIZE: usize> CipherAuthenticated<8>
    for AesCcm<'c, KEY_SIZE, 8, IV_SIZE>
{
}
#[cfg(any(cryp_v2, cryp_v3, cryp_v4))]
impl<'c, const KEY_SIZE: usize, const IV_SIZE: usize> CipherAuthenticated<10>
    for AesCcm<'c, KEY_SIZE, 10, IV_SIZE>
{
}
#[cfg(any(cryp_v2, cryp_v3, cryp_v4))]
impl<'c, const KEY_SIZE: usize, const IV_SIZE: usize> CipherAuthenticated<12>
    for AesCcm<'c, KEY_SIZE, 12, IV_SIZE>
{
}
#[cfg(any(cryp_v2, cryp_v3, cryp_v4))]
impl<'c, const KEY_SIZE: usize, const IV_SIZE: usize> CipherAuthenticated<14>
    for AesCcm<'c, KEY_SIZE, 14, IV_SIZE>
{
}
#[cfg(any(cryp_v2, cryp_v3, cryp_v4))]
impl<'c, const KEY_SIZE: usize, const IV_SIZE: usize> CipherAuthenticated<16>
    for AesCcm<'c, KEY_SIZE, 16, IV_SIZE>
{
}
#[cfg(any(cryp_v2, cryp_v3, cryp_v4))]
impl<'c, const KEY_SIZE: usize, const TAG_SIZE: usize> IVSized
    for AesCcm<'c, KEY_SIZE, TAG_SIZE, 7>
{
}
#[cfg(any(cryp_v2, cryp_v3, cryp_v4))]
impl<'c, const KEY_SIZE: usize, const TAG_SIZE: usize> IVSized
    for AesCcm<'c, KEY_SIZE, TAG_SIZE, 8>
{
}
#[cfg(any(cryp_v2, cryp_v3, cryp_v4))]
impl<'c, const KEY_SIZE: usize, const TAG_SIZE: usize> IVSized
    for AesCcm<'c, KEY_SIZE, TAG_SIZE, 9>
{
}
#[cfg(any(cryp_v2, cryp_v3, cryp_v4))]
impl<'c, const KEY_SIZE: usize, const TAG_SIZE: usize> IVSized
    for AesCcm<'c, KEY_SIZE, TAG_SIZE, 10>
{
}
#[cfg(any(cryp_v2, cryp_v3, cryp_v4))]
impl<'c, const KEY_SIZE: usize, const TAG_SIZE: usize> IVSized
    for AesCcm<'c, KEY_SIZE, TAG_SIZE, 11>
{
}
#[cfg(any(cryp_v2, cryp_v3, cryp_v4))]
impl<'c, const KEY_SIZE: usize, const TAG_SIZE: usize> IVSized
    for AesCcm<'c, KEY_SIZE, TAG_SIZE, 12>
{
}
#[cfg(any(cryp_v2, cryp_v3, cryp_v4))]
impl<'c, const KEY_SIZE: usize, const TAG_SIZE: usize> IVSized
    for AesCcm<'c, KEY_SIZE, TAG_SIZE, 13>
{
}
...
impl<'d, T: Instance, M: Mode> Cryp<'d, T, M> {
    ...
    pub fn start_blocking<'c, C: Cipher<'c> + CipherSized + IVSized>(
        ...
        T::regs()
            .init(0)
            .ivlr()
            .write_value(u32::from_be_bytes(iv_word));
        iv_word.copy_from_slice(&full_iv[iv_idx..iv_idx + 4]);
        iv_idx += 4;
        T::regs()
            .init(0)
    ...
    >(
        ...
        ctx.aad_buffer[ctx.aad_buffer_len..ctx.aad_buffer_len + len_to_copy]
            .copy_from_slice(&aad[..len_to_copy]);
    ...
    pub fn payload_blocking<'c, C: Cipher<'c> + CipherSized + IVSized>(
        ...
        if !last_block {
            if last_block_remainder != 0 {
                panic!(
                    "Input length must be a multiple of {} bytes.",
                    C::BLOCK_SIZE
                );
            }
        }
        ...
        if last_block_remainder > 0 {
            ...
            last_block[..last_block_remainder]
                .copy_from_slice(&input[input.len() - last_block_remainder..input.len()]);
            ...
            ctx.cipher.post_final_blocking(
                T::regs(),
                self,
                ctx.dir,
    ...
    fn load_key(&self, key: &[u8]) {
        ...
        if keylen > 192 {
            ...
            T::regs()
                .key(0)
                .klr()
                .write_value(u32::from_be_bytes(keyword));
            keyword.copy_from_slice(&key[keyidx..keyidx + 4]);
            keyidx += 4;
            T::regs()
                .key(0)
...
impl<'d, T: Instance> Cryp<'d, T, Async> {
    ...
    >(
        ...
        if ctx.aad_buffer_len < C::BLOCK_SIZE {
            // The buffer isn't full and this is the last buffer, so process it as is (already padded).
            if last_aad_block {
                Self::write_bytes(self.indma.as_mut().unwrap(), C::BLOCK_SIZE, &ctx.aad_buffer)
                    .await;
...
pub trait Instance:
    SealedInstance + PeripheralType + crate::rcc::RccPeripheral + 'static + Send
{
