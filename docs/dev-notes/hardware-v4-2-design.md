...
- 官方数据手册（以印字面“正视图”为准）给出的管脚功能为：
  - 1 脚：`GND`（Vin−）。
  - 2 脚：`+Vin`。
  - 3 脚：`−Vo`。
  - 4 脚：`+Vo`。
- v4.2 网表（`analog-board-netlist.enet:gge420`）当前的连接关系为：
  - 引脚 1：`USB-`（上游 5 V 域地，对应 GND / Vin−）。
  - 引脚 2：`PWR_5V`（上游 5 V 电源，对应 +Vin）。
  - 引脚 3：`GND`（隔离次级地，对应 −Vo）。
  - 引脚 4：`5V`（隔离次级 5 V，对应 +Vo）。
- 该模块为 4 引脚 SIP 封装，典型应用为“1/2 输入 + 3/4 输出”，但具体极性与视图方向需严格以 B_S‑1WR3 系列官方数据手册为准。

根据 v4.1 实物板及调试反馈，早期版本曾将 3/4 脚在 PCB 上走线为“3=+5V、4=GND”，与上面的官方定义相反；属于引脚定义与封装/布局之间的对齐错误，而非简单的网络名拼写问题。v4.2 已在原理图与网表中修正为“3=GND、4=5V”，与数据手册保持一致。
