...
- CH1：`I_SENSE_CH1`（新网络名，原本对应 INA193 输出 `CUR1_SP`）；
- CH2：`I_SENSE_CH2`（原 `CUR2_SP`）。

从 OPA2365 输出到 `I_SENSE_CHx` 之间放置一颗 100 Ω 串联电阻（每通道一颗），再由 `I_SENSE_CHx` 向下游分支驱动两个负载：

- 一路：`I_SENSE_CHx` → MOSFET 驱动/电流环路的反相输入（现有原理图中，等价节点为 `$1N154426`/`$1N154901` 一类的 OPA 环路输入）。
- 一路：`I_SENSE_CHx` → MCU ADC 通道，并在 ADC 端就地加小电容到 GND 组成 RC 滤波（详见下节）。
...
1. 删除器件（不装）：
   ...
   - 与 INA193 输出相关的 RC 过滤网络：
     - CH1：R17（1 kΩ）、C14（47 nF）、C13（100 nF）、R18（100 kΩ，可选）及其相关节点（`CUR1_SP`、`$1N91324`、`$1N356` 等）。
     - CH2：R30（1 kΩ）、C25（47 nF）、C24（100 nF）、R31（100 kΩ，可选）及其相关节点（`CUR2_SP`、`$1N91590`、`$1N53239` 等）。
   - 若仅删除 INA193 而保留上述网络，将导致这些节点悬空，应一并在原理图中整理。
