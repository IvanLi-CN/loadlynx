...
- 固件侧通道调度策略（当前实现）：
  - 数字板当前主控制链通过 `SetMode` snapshot 提供**总目标电流** `I_total`（CC 模式下来自 `SetMode.target_i_ma`）；legacy `SetPoint.target_i_ma` 仅在 analog 尚未见过首个合法 `SetMode` 前保留兼容意义；
  - `I_total < 2 A` 时仅驱动 CH1：CH1 目标电流 = `I_total`，CH2 目标电流 = 0；
  - `I_total ≥ 2 A` 时启用两通道并联：CH1/CH2 目标电流近似各为 `I_total / 2`（奇数 mA 由 CH1 多承担 1 mA）；
  - 当前固件中总目标仍在 0–5 A 区间内 clamp，以保持与现有软件/硬件保护阈值一致；10 A 为硬件能力上限，后续若提高总电流目标需要同时调增主控制链的 CC 上限与保护阈值。
...
- 一路：`CURx_SP` → MOSFET 驱动/电流环路的反相输入（原理图中该段节点未单独命名；以“运放反相输入/补偿网络入口”这一结构位置对齐即可）。
