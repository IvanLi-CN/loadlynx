...
**包含：**
- 电压测量多点校准：`v_local_mv` 与 `v_remote_mv`（0–40 V 重点），用户采集 2–24 点，运行时按点集插值修正。
- 电流测量多点校准：总电流 `i_total_ma`（0.5–5 A 重点），用户采集 1–3 点（最多 24 点），运行时插值修正。
...
- **Raw**：STM32 侧在校准模式下附加的**原始采样电压/设定码**（无用户校准），用于采点与前端预览输入；单位设计为便于短帧传输：  
  ...
  - 为提升 DIY 场景抗电源纹波能力，校准模式下 `raw_*_100uv` 在 STM32 侧会先对关键通道做单帧内多次采样平均（默认 256 次）以抑制 1–10 kHz 纹波，再叠加跨 FastStatus 帧的滑动平均（6 帧 ≈ 300 ms，20 Hz），更接近“时间平均 Raw”，而非严格单次瞬时采样值。  
  - 校准模式下屏幕/Web 展示的 `v_*_mv / i_*_ma / p_*_mw` 也基于该平滑后的 Raw 生成，以保证读数稳定一致。  
...
#### 2.2.1 草稿数据清理（Apply Preview / Apply / Commit）

**设计目标：** 用户可自由采集/导入点（允许重复、允许冲突），系统在真正“应用/提交”时再对数据做清理并提示异常，保证下发到 G431 的曲线满足固件约束（可逆、可插值）。

清理规则（按每条曲线独立执行）：
1. **不限制重复点**：草稿中允许同一 `meas`（mV/mA）重复采样、也允许同一 `raw` 重复出现。
2. **重复数据聚合（众数/中位数）**：
   - 对同一 `meas` 的多条样本，聚合出单点：
     - `raw_100uv`（以及电流曲线的 `raw_dac_code`）取 **众数**（若存在“唯一最高频”值），否则取 **中位数**（lower-median，即排序后 `[(n-1)/2]`）。
   - 对同一 `raw_100uv` 的多条样本，聚合出单点：
     - `meas`（以及电流曲线的 `raw_dac_code`）按同样的“众数优先，否则中位数”规则聚合。
3. **单调性修复**：将点按 `raw` 升序排序后，若 `meas` 不是严格递增，则**丢弃冲突点**直到满足严格递增（保证反算/插值可用）。
4. **上限处理**：若清理后点数仍超过 24，则仅保留排序后前 24 点，并在 UI 提示“部分点被丢弃”。

> UI 行为：上述清理/冲突/丢弃会在 **Apply Preview / Apply / Commit** 点击时弹窗提示（例如“重复点已合并”“非单调点已丢弃”“点数超限已截断”），而不是在采集阶段阻止用户继续录入。

运行时对**清理后的点集**与 Raw 读数做**分段线性插值**得到校准值：
...
4. **校准应用职责分配（满足“STM32 自己校准、ESP 不修订”）**
   ...
   - **G431 侧反算（Inverse mapping）算法**：  
     ...
     1) meas 必须严格单调递增；Web/UI 允许对同一 meas 重复采样，但在下发前会先按 2.2.1 的规则用“众数/中位数”聚合并丢弃冲突点，再做验证。  
...
2. 用户将电源调到一个电压点（如 12 V / 24 V / 36 V）。
...
5. **标定点列表**：可删除点；显示每点的 raw、meas、点位编号；**不限制重复与冲突**（重复点会作为多条样本保留，清理与提示发生在 Apply Preview / Apply / Commit）。
...
前端维护每个 Tab 的草稿点数组。用户点击 **Apply Preview** 时，前端按 2.2.1 的规则清理草稿点并生成 `candidate_points[]`（同时提示冲突/异常），预览值的计算为：
