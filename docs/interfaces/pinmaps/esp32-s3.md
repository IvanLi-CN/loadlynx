...
| 43 | GPIO38 | RGB_R_PWM | RGB（共阳/COM=3V3）红通道 PWM，GPIO 灌电流（active-low）；直驱需串联限流电阻（spec docs/specs/touch-spring-load-switch-rgb-led/SPEC.md，详见 `docs/interfaces/touch-switch-and-rgb-led.md`）。 |
| 44 | MTCK (GPIO39) | RGB_G_PWM | RGB（共阳/COM=3V3）绿通道 PWM，GPIO 灌电流（active-low）；直驱需串联限流电阻（spec docs/specs/touch-spring-load-switch-rgb-led/SPEC.md，详见 `docs/interfaces/touch-switch-and-rgb-led.md`）；需禁用 PAD-JTAG。 |
| 45 | MTDO (GPIO40) | RGB_B_PWM | RGB（共阳/COM=3V3）蓝通道 PWM，GPIO 灌电流（active-low）；直驱需串联限流电阻（spec docs/specs/touch-spring-load-switch-rgb-led/SPEC.md，详见 `docs/interfaces/touch-switch-and-rgb-led.md`）；需禁用 PAD-JTAG。 |
...
| 18 | GPIO13 | CS | 已用 | 同上。 |
| 19 | GPIO14 | TOUCH_SPRING | 规划 | Touch Sensor：`TouchPad14`；电极为触摸弹簧，顶住亚克力背面（0.8–1.0 mm）；固件用作触摸电源键/本地输入（熄屏时用于唤醒，spec docs/specs/touch-power-button-standby-breathing-white/SPEC.md）；注意与 `I2S_BCLK` 等高速时钟、RGB PWM 走线隔离（spec docs/specs/touch-spring-load-switch-rgb-led/SPEC.md，详见 `docs/interfaces/touch-switch-and-rgb-led.md`）。 |
...
| 39 | GPIO34 | AMP_SD_MODE | 规划 | MAX98357A：`SD_MODE`（shutdown + channel select）；固件可用作 AMP_EN（低=shutdown；高=Left）（spec docs/specs/touch-spring-load-switch-rgb-led/SPEC.md）。 |
| 40 | GPIO35 | I2S_BCLK | 规划 | MAX98357A I²S：位时钟（BCLK）；三线占用需连续封装引脚（spec docs/specs/touch-spring-load-switch-rgb-led/SPEC.md）。 |
| 41 | GPIO36 | I2S_LRCLK | 规划 | MAX98357A I²S：字选择/左右时钟（LRCLK）；三线占用需连续封装引脚（spec docs/specs/touch-spring-load-switch-rgb-led/SPEC.md）。 |
| 42 | GPIO37 | I2S_DIN | 规划 | MAX98357A I²S：数据输入（DIN，ESP→AMP）；三线占用需连续封装引脚（spec docs/specs/touch-spring-load-switch-rgb-led/SPEC.md）。 |
| 43 | GPIO38 | RGB_R_PWM | 规划 | RGB（共阳/COM=3V3）红通道 PWM，GPIO 灌电流（active-low）；直驱需串联限流电阻；连续封装引脚（Pin 43/44/45）（spec docs/specs/touch-spring-load-switch-rgb-led/SPEC.md，详见 `docs/interfaces/touch-switch-and-rgb-led.md`）。 |
| 44 | MTCK / GPIO39 | RGB_G_PWM | 规划 | RGB（共阳/COM=3V3）绿通道 PWM，GPIO 灌电流（active-low）；直驱需串联限流电阻；默认 JTAG TCK，需禁用 PAD-JTAG；连续封装引脚（Pin 43/44/45）（spec docs/specs/touch-spring-load-switch-rgb-led/SPEC.md，详见 `docs/interfaces/touch-switch-and-rgb-led.md`）。 |
| 45 | MTDO / GPIO40 | RGB_B_PWM | 规划 | RGB（共阳/COM=3V3）蓝通道 PWM，GPIO 灌电流（active-low）；直驱需串联限流电阻；默认 JTAG TDO，需禁用 PAD-JTAG；连续封装引脚（Pin 43/44/45）（spec docs/specs/touch-spring-load-switch-rgb-led/SPEC.md，详见 `docs/interfaces/touch-switch-and-rgb-led.md`）。 |
