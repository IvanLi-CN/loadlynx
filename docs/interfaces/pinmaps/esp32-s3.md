...
| 18 | GPIO13 | CS | 已用 | 同上。 |
| 19 | GPIO14 | TOUCH_SPRING | 已用 | Touch Sensor：`TouchPad14`；电极为触摸弹簧，顶住亚克力背面（0.8–1.0 mm）；固件：去抖/校准窗口 + cooldown；注意与 RGB PWM 走线隔离（Plan #0021）。 |
...
| 40 | GPIO35 | RGB_R_PWM | 已用 | RGB（共阳/COM=3V3）红通道 PWM，GPIO 灌电流（active-low）；固件：LEDC Timer3 + Ch3；未被内置 Flash/PSRAM 占用（Plan #0021）。 |
| 41 | GPIO36 | RGB_G_PWM | 已用 | RGB（共阳/COM=3V3）绿通道 PWM，GPIO 灌电流（active-low）；固件：LEDC Timer3 + Ch4；未被内置 Flash/PSRAM 占用（Plan #0021）。 |
| 42 | GPIO37 | RGB_B_PWM | 已用 | RGB（共阳/COM=3V3）蓝通道 PWM，GPIO 灌电流（active-low）；固件：LEDC Timer3 + Ch5；未被内置 Flash/PSRAM 占用（Plan #0021）。 |
| 43 | GPIO38 | — | 空 | 可用 IO。 |
