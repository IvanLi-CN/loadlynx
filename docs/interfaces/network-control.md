...
本文档描述基于 ESP32‑S3 Wi‑Fi 能力与现有 Web 控制台的整体需求与当前实现边界，用于支撑多设备发现、统一负载控制、状态/诊断查看、USB‑PD 配置、校准与固件维护。

## 1. 背景与目标

- 数字板：ESP32‑S3 作为主控，现有功能包含本地显示 UI、旋钮输入与与模拟板（STM32G431）之间的 UART 协议链路。
- 新目标：
  - 为 ESP32‑S3 提供可靠的 Wi‑Fi STA 联网能力，以及覆盖控制、状态、配置和维护场景的 HTTP+JSON API。
  - 在仓库根目录的 `web/` 下维护一个多设备 Web 控制台，用于设备管理、统一负载控制、状态诊断、USB‑PD、校准与固件操作。
  - 保持现有本地 UI 与 UART 协议行为不变，在其基础上叠加网络控制与维护能力。
...
- Web 控制台：
  - 多设备列表 / 管理界面。
  - 单设备控制台界面，包括负载控制、状态、USB‑PD、设置、校准与固件页面。
  - 使用前端 Mock 完成开发与交互验证，并可切换到真实 ESP API。

暂不在范围内（可以作为后续扩展）：

- 自动化局域网扫描 / mDNS 发现作为 Web 主发现路径（当前版本仍以“手动添加设备 + 连通性检测”为主）。
- 修改模拟板（STM32G431）固件协议字段（例如新增真正的“最小维持电压”参数）。
- 完整的认证/鉴权（当前假定在受控局域网内使用）。
- 云端设备注册、跨浏览器同步的设备目录服务。

## 3. 总体架构概览

- 硬件侧：
  - ESP32‑S3 通过 Wi‑Fi 以 STA 模式接入指定 AP，并运行 HTTP 服务器。
  - ESP32‑S3 通过现有 UART 协议（`loadlynx-protocol`）与 STM32G431 交互，维持 `FastStatus` 遥测与 `SetMode`/`LimitProfile`/`SoftReset`/`SetEnable`/PD request 控制闭环；legacy `SetPoint` 仅保留兼容路径。
- 软件侧：
  - 新 Web App（`web/`）通过 HTTP 调用 ESP32‑S3 暴露的 REST 风格 API。
  - Web App 使用 TanStack Router 管理路由，TanStack Query 管理数据请求与缓存，支持多设备与多功能子路由。
  - 前端可通过 Mock 层或 devd/dev API 完成交互设计、状态管理与浏览器测试。
...
- 顶层路由形态：
  - 根入口：`/`，当前复用设备列表视图。
  - 设备发现与管理页：`/devices`
  - 单设备功能页：`/:deviceId/<subroute>`
    - 当前实现包含：
      - `/:deviceId/cc`：主控制台，承载统一负载控制、监控读数、限值、预设与诊断摘要；
      - `/:deviceId/status`：详细状态与原始遥测/PD 摘要；
      - `/:deviceId/pd`：USB‑PD 能力与请求配置；
      - `/:deviceId/settings`：Wi‑Fi、soft reset、诊断导出、备份恢复；
      - `/:deviceId/calibration`：标定模式、曲线编辑与实时状态流；
      - `/:deviceId/firmware`：通过 devd 或 Web Serial 的固件维护入口。
- 路由优先级与冲突避免：
...
- 路由行为：
  ...
  - 从 `/devices` 点击某设备进入具体功能时，默认跳转到 `/:deviceId/cc`，也可进入固件等子路由。

### 4.5 主控制台（`/:deviceId/cc`）

- 数据来源：
  - 来自 ESP API 的：
    - `FastStatus` 摘要（电压/电流/功率/温度/故障状态）。
    - `GET /api/v1/control` 返回的统一控制真相源。
    - `GET /api/v1/cc` 返回的 CC convenience view。
    - `GET /api/v1/pd` 返回的 PD 摘要信息。
    - `GET /api/v1/identity` 返回的能力与版本信息。
  - 页面通过 TanStack Query：
...
- UI 结构（主视图）：
  - 上半部分：状态总览
    ...
    - 展示运行模式、故障标志、温度等概要信息。
  - 下半部分：控制区
    - 控制模式选择：基于能力决定可编辑模式，当前支持 CC/CV，若固件声明支持则开放 CP；legacy CR 只读显示，不作为编辑主路径。
    - 输出开关：映射统一控制视图中的 `output_enabled`，由 digital 侧串行化到 `SetMode` 主控制链，并在链路/故障/UVLO 等条件下做 enable gating。
    - 设定值编辑：
      - CC：目标电流；
      - CV：目标电压；
      - CP：目标功率（仅在固件能力允许时）；
      - 配套软限值包括 `max_i_ma_total`、`max_p_mw`、`min_v_mv`。
    - 预设区：读取/编辑/应用 preset 槽位，并把“应用 preset 会关闭输出”作为显式交互约束。
    - 诊断/状态摘要：展示链路、故障、最后一次 apply 结果与 PD 合同摘要。
- 交互行为：
  - 本地草稿状态与设备状态分离：
    - 本地 UI 即时更新预览值；
    - 控制命令通过 mutation 提交，成功后回填 query cache，避免等待下一轮轮询。
  - 错误状态（如链路掉线、模拟板故障、API 不支持）在页面上高亮展示，并禁用相关控件。
  - 页面对可见性敏感：页面不可见时暂停高频轮询，避免无意义刷新。

### 4.6 Mock 接口与前端开发

- Mock 目标：
  - 在 ESP API 不可用或需要前端独立开发时，前端可以独立完成 UI/路由/状态管理及大部分交互。
- 建议方案：
...
- 当前控制模型（内部）：
  - 数字板固件维护两层视图：
    - `/api/v1/control` 暴露 **v1 frozen** 的 active preset / output 开关真相源：
      - `active_preset_id`
      - `output_enabled`
      - `uv_latched`
      - `preset`（`mode`、`target_i_ma`、`target_v_mv`、`target_p_mw`、`min_v_mv`、`max_i_ma_total`、`max_p_mw`）
    - `/api/v1/cc` 暴露向后兼容的 **CC-focused convenience view**：
      - `enable`
      - `target_i_ma`
      - `effective_i_ma`
      - `limit_profile`
      - `protection`
      - 关键测量摘要
  - 当前真实 UART 下行控制由 `ControlState` 派生，并统一串行化到 `SetMode` 主路径；`SetPoint` 不再是 Web/API 控制面的主下行消息。
  - `SetEnable` 仍参与启动/恢复握手与模拟侧 gating，但 Web 主控制开关的语义已经是 `output_enabled` / load switch，而不是“直接发一个独立 SetEnable 就完成控制”。
- 当前主要端点：
  - `GET /api/v1/control`
    - 返回统一控制真相源：
      - `active_preset_id`
      ...
      - `preset`
  - `POST /api/v1/control`
    - 当前 Web 主路径；更新统一控制真相源，例如输出开关、活动 preset 或 preset 内容。
    - `PUT /api/v1/control` 仍由 firmware 兼容接受同一 payload。
  - `GET /api/v1/cc`
    - 返回当前 CC 兼容视图：
      - `enable`；
      - `target_i_ma`；
  ...
  - `PUT /api/v1/cc`
    ...
    - 行为：
      ...
      - 将兼容视图收敛为当前 `ControlState`，再由统一 UART TX 路径派生 `SetMode` / `LimitProfile` / 必要的恢复握手；
      - 返回更新后的完整 CC 状态。
- 与控制链并列的已实现辅助端点：
...
- `GET /api/v1/diagnostics/export`
  - 提供 redacted diagnostics 快照，适合 Web 导出或运维抓取。
  - `GET /api/v1/diagnostics` 目前仍是同 payload 的兼容别名。
- `GET/POST/DELETE /api/v1/wifi`
  - 提供 Wi‑Fi 状态、配置与清除入口。
- `GET /api/v1/wifi/credentials`
  - 仅用于显式备份/导出路径。
- 标定与固件维护：
  - 标定通过单独端点和状态流完成；
  - 固件刷写由 Web 使用 devd/Web Serial 侧路径完成，而非直接通过 HTTP API 传输固件镜像。
...
- Query key 约定：
  ...
  - 统一控制视图：`["device", deviceId, "control"]`。
  - CC 控制视图：`["device", deviceId, "cc"]`。
  - PD 视图：`["device", deviceId, "pd"]`。
  - Wi‑Fi 状态：`["device", deviceId, "wifi"]`。
  - 设备列表（本地存储）：`["devices"]`。
- 轮询与刷新：
...
- 在 `/:deviceId/...` 页面：
  - 首次进入时先加载 `/identity`，校验设备类型、能力和协议版本；
  - 根据页面类型启动 `/status`、`/control`、`/cc`、`/pd`、`/wifi` 或标定相关 Query；
  - 若发现协议版本不兼容或 API 缺失，应显示“版本不匹配 / 功能不支持”提示，并在可行时引导用户进入固件升级页面。

## 8. 实施与验证要点

- 前端：
  - 维持路由、设备存储、Mock、Storybook 与浏览器测试之间的一致性；
  - 确保 CI（lint/check/build/test:e2e）在本仓库通过；
  - 在桌面视口下验证主控制台、状态、PD、设置、校准和固件页面的基本交互。
- 固件：
  - 在开发板上验证 Wi‑Fi 连接的稳定性与重连行为；
  - 确认 HTTP API 响应时间不会明显干扰 UART 链路与 UI 刷新；
  - 确认控制、PD、Wi‑Fi、诊断与标定端点在链路异常时返回可区分的错误码。
- 联调：
  - 先用 Mock 或 devd 兼容设备完成前端逻辑，再切换为真实设备并对照 FastStatus/UI 行为；
  - 对典型使用场景（正常运行、过流/过温、链路掉线、PD 不可用、Wi‑Fi 未配置、校准模式切换）编写简单的手工测试用例，以备后续 PR 中引用。
