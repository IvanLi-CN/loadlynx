...
- 消息与标志：继续使用 `MSG_SET_POINT`，请求帧带 `FLAG_ACK_REQ`，确认帧带 `FLAG_IS_ACK`（`FLAG_IS_NACK` 可选用于回报解析失败）。
- 确认格式：
  - 头部 `seq`/`msg` 回显；`len=0` 时 payload 为空（可选回显 `target_i_ma` 便于调试）。
  - CRC 仍覆盖头部与 payload。
- 数字侧（ESP32‑S3）发送侧状态机：
  - 发送 SetPoint 后进入等待 ACK 状态，超时 30–50 ms 未收 ACK 则按 30/60/120 ms 退避重发，最多 3 次。
  - 收到匹配 `seq`/`msg` 的 ACK 即清除等待；若收到 NACK 则立即重发或提示错误。
  - 新目标出现时应“最新值优先”：可取消旧帧、使用新 `seq` 立即发送，重置等待计时。
- 模拟侧（STM32G431）接收侧：
  - 成功解析并应用 SetPoint 后立即回 ACK；若 `seq` 与上次相同，则视为重传，仅回 ACK 不重复应用（幂等）。
  - 解析错误时可回 NACK，附错误码或最少头部。
- 一致性自检：数字侧周期比对遥测中的 `target_value` 与本地期望值；连续不一致时提示 UI 并强制重发最新 SetPoint。
- 软复位协同：软复位握手完成后双方重置与 SetPoint 相关的 `seq` 记忆，避免旧重传被误判。
