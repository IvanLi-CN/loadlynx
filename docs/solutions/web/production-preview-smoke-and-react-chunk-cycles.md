...
- If the stale client is still controlled by the old service worker, gate `clientsClaim` and `skipWaiting` behind an explicit migration-build flag. Ordinary releases must return to prompt-style activation after the migration build.
