...
- **阶段 0（已冻结）**：在绘制新的触屏目标值界面前，先把“右下角电压/电流设置区域”彻底清空，避免旧控件干扰新的布局讨论。
  - 本计划的 3 张效果图中，右下角必须**不出现**任何电压/电流设置控件（包括 `Vreq/Ireq` 标签、步长、输入框、单位、`+/-` 按钮、编辑下划线等）。
  - 允许保留：右栏的 `Active contract` / `Selected` 信息卡片，以及底部 `Back` / `Apply` 按钮。
  - （像素契约）清空区域：`x=190..319`，`y=130..205`（精确覆盖旧的 `Vreq/Ireq` 行与 `+/-` 控件，避免裁切 `Selected` 卡片与底部按钮）。
  - 清空区域的背景应与页面背景一致（以 `#0B111E` 为基准），不得出现残留控件边缘或高亮描边。
