...
- 目前 digital 固件在启动时将背光 PWM 设为固定亮度（当前为 80%），之后缺少“无操作自动熄屏/调暗”的策略。
...
- 在 **LOAD OFF** 且**无用户操作**时，自动进入分级省电：
  ...
  - 之后 `+3min` 仍无操作：关闭背光，并将屏幕内容刷新为纯白；进入 display sleep（`Display::sleep()`）以降低面板功耗。
...
- **熄屏行为**
  - 进入熄屏时：先将屏幕内容刷新为纯白（确保面板 RAM 为白），再关闭背光（duty=0）。
...
- Given 设备处于 `LOAD OFF` 且持续无用户输入
  When 到达该时刻后 5 分钟
  Then 在 1 秒内完成“屏幕内容刷新为纯白 → 关闭背光（0%）→ 进入 display sleep”，并打印一次状态切换日志。
...
- [ ] M2: 设计并落地屏幕电源状态机（Active/Dim/Off）与背光单点控制方案
- [ ] M3: 验收与可观测性：日志、硬件验证步骤、边界用例确认
...
| Dim | `LOAD OFF` 且 `idle ≥ 5min` | Off | 刷白屏 → `backlight = 0` → `Display::sleep()` |
...
1. 任意输入均视为“有操作”，重置计时。
2. 熄屏后任何输入仅用于唤醒且会被消费。
3. 熄屏前刷新纯白、随后关闭背光（白底用于防残影/避免旧画面闪烁）。
