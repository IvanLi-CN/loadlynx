# LoadLynx operational skills packaging and workflow boundary
...
- 保持两个核心 operational skill，并允许仓库同时公开 release companion skill：
  ...
  - `loadlynx-release-decision`：release 决策 companion skill；不属于普通用户主指南，但可以作为同仓公开 skill 被发现与安装。
- 普通用户的电脑安装/更新主指南固定为 `docs/user/install-and-update.md`：`README.md` 只保留短入口，程序使用 GitHub Release installer + `SHA256SUMS`，`loadlynx-user-operations` skill 使用全局安装与定向全局更新命令。
- 用户版覆盖 released `loadlynx` CLI 硬件操作、USB/devd 优先连接、HTTP fallback、CLI 硬件记忆、设备身份/状态/遥测、电子负载输出与模式/预设/设定值、USB-PD、WiFi `show|set|clear`、GitHub Release 固件下载与 CLI 烧录；所有流程仍必须先以 installed CLI help 验证当前命令真实存在。
...
- 不把缺失的 installed CLI 命令写成可用功能；release 文档只能描述当前稳定 release 已公开的命令面。
- 不在 skill 文档内设计完整桌面安装器、代码签名、公证或自动更新。
- 不在本 spec 直接新增固件协议、devd API 或 CLI 命令；这些属于开发者 skill 引导的后续源码工作。
...
- `docs/user/install-and-update.md`
...
- `loadlynx`：`host-tools` 内的 released CLI。当前稳定 release 公开 owner-facing 命令包括 `completion`、`devices`、`device`、`status`、`flash`、`cc` / `cv` / `cp`、`pd`、`wifi`、`control`、`preset`；`usb-port set`、`reset`、`monitor` 等维护命令可存在但应隐藏。是否包含具体业务能力仍必须以用户安装版本的 `loadlynx --help` 为准。
- `程序`：GitHub Releases 发布的 `loadlynx` 与 `loadlynx-devd` host tools。
- `skill`：在普通用户安装/更新指南里特指 `loadlynx-user-operations`。
- 外部 USB-C source validation：LoadLynx 作为通用验证 sink 时，通过 `loadlynx pd set` 产生 PD sink 请求，通过 `loadlynx cv <target_v_mv>` 产生电压钳位负载刺激；被测 source device 自己的诊断是电流限制或 fault verdict 的主来源，LoadLynx telemetry 是辅助交叉证据。
- CLI 设备记忆：`loadlynx devices` 与 `loadlynx device list|add|use|remove` 维护用户级设备 registry，以稳定 `identity.device_id` 为主键，覆盖已绑定设备、全局默认、每台设备的 USB/HTTP transport locator 与 `last_transport`。默认路径为 macOS `~/Library/Application Support/LoadLynx/devices.json`，Linux `${XDG_CONFIG_HOME:-~/.config}/loadlynx/devices.json`，Windows `%APPDATA%\\LoadLynx\\devices.json`；`LOADLYNX_HOME` 可覆盖目录。本地目录选择文件为最近祖先 `.loadlynx` 点文件，内容仅为单个 saved device id。
...
- `vercel-labs/skills` 必须能从仓库发现并安装公开的 LoadLynx skill；其中核心 operational skill 为 `loadlynx-user-operations` 与 `loadlynx-developer-operations`。
- 用户版不得要求源码 checkout、`just`、Rust、Bun、外部 MCU daemon、`espflash` 或 `probe-rs`。
...
- 普通用户公开安装文档必须把 skill 默认写成全局安装：`npx skills add https://github.com/IvanLi-CN/loadlynx --skill loadlynx-user-operations -g`。
- 普通用户公开更新文档必须把 skill 默认写成定向全局更新：`npx skills update loadlynx-user-operations -g`；不得再把重新执行 `npx skills add` 写成 skill 更新路径。
...
- 用户版连接顺序必须是 USB/devd 优先，HTTP 其次。HTTP 只在 USB 不可用、用户明确选择 HTTP、或使用已保存 HTTP 设备时作为 fallback。该 fallback 不适用于 runtime WiFi 写入：`wifi set`、`wifi clear` 和包含 `settings.wifi` 的 restore 必须使用 USB/devd 等独立控制路径，不能通过 LAN HTTP、mDNS、saved HTTP transport 或 "verified LAN" 继续。
- 用户版必须要求 CLI 先绑定设备再操作设备。当前实现使用 `loadlynx devices`、`loadlynx device list|add|use|remove`、`loadlynx status` 和 `loadlynx status --device <id>`；同一设备同时存在 USB 与 HTTP transport 时按 `last_transport` 选择。临时 USB/devd candidate ID 只能通过 `loadlynx device add` 的交互选择或 `--usb-port <path>` 显式端口绑定流进入设备记忆，不能直接用于 status、控制、诊断、烧录或监控。
- 用户版必须按业务功能组织硬件操作：身份与状态确认、遥测查看、电子负载输出控制、CC/CV/CP/预设、USB-PD、固件更新、WiFi 配置和外部 USB-C source validation。连接方式和硬件记忆只是进入业务操作的前置步骤。
...
- 用户版必须覆盖 released CLI WiFi 配置入口：`loadlynx wifi show|set|clear`。执行前仍必须确认当前 installed CLI 暴露这些命令；如果命令不存在，必须明确报告该安装版本不支持，不得伪造命令或回退 raw HTTP。
...
- 开发者版必须保留项目开发端口/探针缓存、devd lease、WiFi secret、校准、flash/reset/monitor/logs 与 HIL 的安全门禁；具体缓存文件名只在开发者 skill 中列出，所有硬件操作职责归 `loadlynx` CLI + `loadlynx-devd`。
...
- 普通用户安装/更新入口文档：`docs/user/install-and-update.md`。
...
- WiFi：使用 released `loadlynx wifi show|set|clear` 命令；保护 PSK，不把密钥写入聊天、日志、截图、trace 或 PR。WiFi 写入必须走 USB/devd 等独立管理路径；如果当前 saved device 只有 LAN HTTP transport，agent 必须先引导绑定同一设备的 USB/devd，不能使用 LAN HTTP override 或把 "verified LAN" 当作可用替代连接。
- 外部 USB-C source validation：使用 saved device 和 USB/devd 优先路径；通过 `pd set` 选择 fixed/PPS sink stimulus，通过 `cv` 施加电压钳位负载，且只把 LoadLynx telemetry 当作外部 DUT 诊断的辅助证据。
- 禁止项：clone、`just`、源码构建、外部 MCU daemon、probe selector、HIL、校准写入、raw HTTP 绕过、手改缓存。

### 开发者技能

- 适用对象：开发、维护、调试、发布验证与 HIL。
- 默认入口：验证 checkout；没有 checkout 且任务需要源码时 clone canonical repo。
- 允许项：构建、测试、devd、CLI、Web、release workflow、USB CDC 证据、固件烧录、WiFi 实现/配网、校准、reset/monitor/logs、HIL；硬件操作仍必须通过 `loadlynx` CLI + `loadlynx-devd`。
...
- CLI Hardware Memory:
  ...
  - CLI 支持 `loadlynx devices`、`loadlynx device list|add|use|remove`、`loadlynx status` 和 `loadlynx status --device <id>`。
  - `loadlynx device add` 在交互 TTY 中扫描并选择 USB 设备；`loadlynx device add --usb-port <path>` 非交互绑定指定 USB CDC 端口；`loadlynx device add --url ...` 通过 HTTP identity 绑定同一设备的 LAN transport。
  - `loadlynx device use <id>` 写入最近祖先 `.loadlynx`；`loadlynx device use --global <id>` 选择 selector-free 全局默认；`--clear` 清除对应选择。
  - 硬件记忆位于用户配置目录，`LOADLYNX_HOME` 可用于测试或高级覆盖。
  - 已保存同一硬件同时存在 USB 与 HTTP 时使用 `last_transport`，用户明确选择 HTTP 时才走 HTTP fallback。
...
- User Workflows:
  ...
  - CLI WiFi 配置：`wifi show|set|clear`，必须先确认 installed CLI 真实存在对应命令。`wifi show` 可走可用 read path；`wifi set|clear` 必须使用 USB/devd 等独立控制路径，不能通过 LAN HTTP fallback 写入。
  - 外部 USB-C source validation：`pd set` + `cv` 组合刺激，DUT diagnostics 为主判定，LoadLynx telemetry 为辅助证据。
- Escalate Out:
  - 缺失 CLI/devd 功能、源码构建、release workflow、`just`、HIL、校准、协议/API 实现进入开发者技能。
...
- devd And USB CDC:
  - 使用 `loadlynx-devd`，不走外部 MCU daemon 或 selector。
...
- Given 仓库包含公开的 LoadLynx skills
  When 运行 `npx skills add . --list`
  Then 输出至少包含 `loadlynx-user-operations`、`loadlynx-developer-operations`，以及仓库当前公开的其他 companion skills。
- Given 一个空临时目录
  When 使用 `npx skills add https://github.com/IvanLi-CN/loadlynx --skill loadlynx-user-operations --skill loadlynx-developer-operations -g`
...
- Given 用户请求查找曾经连接过的硬件
  When installed `loadlynx --help` exposes `devices`, `device`, and `status --device`
  Then agent lists saved devices through `loadlynx devices` or `loadlynx device list` and uses `--device <id>` before manual scanning.
...
- Given 用户请求 CLI WiFi 配置
  When installed `loadlynx --help` exposes `wifi` and `loadlynx wifi --help` exposes `show|set|clear`
  Then agent may use `loadlynx wifi show|set|clear --device <saved-device-id>` while protecting PSK from logs and chat.
- Given 用户请求 CLI WiFi 写入
  When the saved device only has LAN HTTP reachability
  Then agent must not use LAN HTTP, mDNS, saved HTTP transport, "verified LAN", or an unsafe override to write WiFi; it must ask the user to bind/use USB/devd for the same device before `wifi set|clear`.
- Given 用户请求 CLI WiFi 配置
  When installed `loadlynx --help` does not expose `wifi`
  Then agent 明确报告当前安装版本不支持 CLI WiFi 配置，并升级到开发者实现/发布任务。
- Given 外部 USB-C source 电流限制验证
  When saved LoadLynx hardware is available through USB/devd transport
  Then agent uses `loadlynx pd set` for PD sink stimulus, `loadlynx cv <target_v_mv>` for voltage-clamp load stimulus, treats DUT diagnostics as the primary verdict, and uses LoadLynx status telemetry only as auxiliary evidence.
...
- 两个核心 operational skill 目录都必须保留 `SKILL.md` 与 `agents/openai.yaml`。
- `npx skills add . --list` 可发现两个核心 operational skill，并允许同时列出仓库公开的 companion skills。
- `git diff --check` 通过。
- 仓库不保留旧 USB/devd skill 路径引用。
- skill 不得把 installed CLI 缺失的命令或缺失的 Release firmware catalog 写成已可用功能。
- repo quality gates must reject stale docs that reintroduce obsolete user CLI daemon URL paths, remove the released WiFi command surface, or mention a project-specific external DUT name in LoadLynx-owned docs.

## 风险与开放问题

- 当前 release host-tools 是压缩包中的 CLI/daemon binaries，不是签名桌面安装器。
- 当前 stable release `v0.5.1` 已公开 `loadlynx wifi show|set|clear`；未来文档仍必须以 installed CLI help 为准，不能假设旧安装版本也支持。
