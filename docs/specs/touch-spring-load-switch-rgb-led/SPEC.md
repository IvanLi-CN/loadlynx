...
- 与既有“负载开关语义”保持一致：复用 `docs/specs/cc-load-switch-toggle/SPEC.md` 的控制模型（`set_*` 与 `load_enabled` 分离；`effective_*` 派生）。
...
- 不修改模拟板（STM32G431）控制闭环与功率链路；负载开关仍按 `docs/specs/cc-load-switch-toggle/SPEC.md` 通过“下发生效值=0”实现。
...
- 触摸按键触发的 `load_enabled` 切换行为必须与 `docs/specs/cc-load-switch-toggle/SPEC.md` 一致：
  - 当“设置值=0”时不得触发开启（仍遵循 `docs/specs/cc-load-switch-toggle/SPEC.md` 的强制安全规则）。
...
- Given 设备上电且 `set_* > 0` 且 `load_enabled=false`，
  When 触摸弹簧被一次“短触”触发，
  Then `load_enabled` 变为 `true`，并在下一次下发中让 `effective_*` 生效（按 `docs/specs/cc-load-switch-toggle/SPEC.md` 的规则计算）。
- Given `set_* == 0`，
  When 触摸弹簧被触发，
...
- 触摸按键：
  ...
  - 触发动作复用既有“负载开关 toggle”路径，确保与 `docs/specs/cc-load-switch-toggle/SPEC.md` 的安全规则一致；并且不依赖旋钮按键路径（旋钮按键在本计划中不要求可用）。
- RGB LED：
...
- `docs/specs/cc-load-switch-toggle/SPEC.md`
