...
- Preset 数据模型与安全语义基线：`docs/specs/0002-cv-mode-presets/SPEC.md`
- Preset UI 保护字段命名与三线约束：`docs/specs/0006-preset-ui-protection-labels/SPEC.md`
...
- 该图标必须被“圆形按钮容器”包裹（按钮本体有独立背景与边框），并留出合适的内边距（symbol 不贴边），避免看起来像裸图标。
- 按钮容器必须看起来“真正是圆的”：禁止任何会产生“重采样错位感”的元素（例如斜线/细线抗锯齿/不对齐的直线高光）；只允许像素级圆环与对齐到像素网格的线段。
- 为避免“中心落在像素之间”导致的视觉偏移，建议按钮容器使用**奇数像素**的直径（例如 `27×27`，约为原 `21×21` 的 1.3×）并保证居中对齐。
- 电源 symbol 的竖线建议为**单像素居中**（避免 2px 宽度导致左右间距不一致的错觉）。
...
- 用户可见语义（对外缩写）：
  - `UVLO`：欠压（阈值为当前预设的 `min_v_mv`）：
    - **Enable 预检拒绝**（启动前就欠压）：当用户尝试启用 `LOAD` 且 `V_main ≤ min_v_mv`，拒绝启用（播放一次 `UI fail` 失败音），**不进入连续告警**；右下角短暂显示 `UVLO`（闪烁，用于解释无法启用；例如持续 ~3s）。
      ...
      - 若用户希望“先打开 LOAD 再接入被测设备”，可将 `UVLO(min_v_mv)` 设为 `0` 以禁用该预检（见 `docs/specs/0006-preset-ui-protection-labels/SPEC.md`）。
...
建议的节奏基线（实现可在 `docs/specs/0007-prompt-tone-manager/SPEC.md` 的 `FaultAlarm` 基础上扩展；需实机试听后微调，但必须保证 primary/secondary 可区分）：
...
- 横向滑动选位（一次手势一位）：
  - 在 Setpoint 按钮内左右滑动可移动编辑位，每次手势最多移动 **1** 位（不循环）；跨多位时重复滑动即可。
  - 编辑位成功移动 **1** 位：播放 **detent tick**（必须可数）。
...
- 触屏负责选择“当前正在编辑的字段”，并可直接调整当前编辑位（光标位）：
  - 点击字段行：仅选择字段（不改变编辑位）。
  - 在字段**值区域**点按：选择字段，并按点按位置选择编辑位（与主界面 Setpoint 点按选位规则一致：小数点吸附、不可选位吸附到最近可选位）。
  - 在字段**值区域**横向滑动：选择字段，并移动编辑位（与主界面 Setpoint 横向滑动选位一致：每次手势最多移动 1 位；成功移动播放 detent tick；越界 UI fail 且每手势最多一次）。
...
> `UVLO` / `OCP` / `OPP` 对应 `docs/specs/0002-cv-mode-presets/SPEC.md` 的字段语义；命名与约束详见 `docs/specs/0006-preset-ui-protection-labels/SPEC.md`。
...
- 主界面 Setpoint：点击直达选位（UI ok）；横向滑动选位（每手势最多移动 1 位；成功移动播放 detent tick）；越界 UI fail（每手势最多一次）；电流/电压/功率显示均为固定宽度格式；可选位限制按本文规定生效。
- 设置面板 tabs：位于面板内左侧纵向排列；单击 tab 仅切换 editing；双击“当前 tab”才激活并关断负载。
- 设置面板字段值区域：点按/横向滑动会自动选中字段并选位（规则同主界面 Setpoint）。
- 保存按钮无改动时置灰禁用且点击仅 UI fail（不写 EEPROM）；保存成功/失败 toast；保存失败阻塞且只能重复点同一保存按钮重试。
- 保存失败阻塞态必须显示持续可见的解释文案（例如 `SAVE FAILED` + `RETRY SAVE`）。
- 未保存改动丢弃规则按本文冻结规则生效。
- 所有 UI 文案中 `LOAD` 一词保持一致，不出现 `OUTPUT`。
- 实现完成后应同步更新本文相关段落，并重新生成 UI mock PNG（至少 `docs/assets/on-device-preset-ui/preset-panel-output-{off,on}.png`），确保与本文一致。
