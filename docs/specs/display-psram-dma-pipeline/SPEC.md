# Digital Display PSRAM/DMA Pipeline

## 背景 / 问题陈述

- 早期 digital 固件以单个 render 任务串行完成 UI 绘制与 SPI 推屏，并在默认 `net_http` 构建中退化为单 framebuffer + 整帧分块推送。
- 该结构让 render/push 与 UART/Wi-Fi 争用 ESP32-S3 内部资源，也无法稳定利用局部刷新。
- 本规格冻结“PSRAM 专用多缓冲 + DMA present 管线”的长期行为与验收边界，不改变 UI 视觉或业务语义。
...
- `docs/specs/uart-comm-troubleshooting/SPEC.md`
