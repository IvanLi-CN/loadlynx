...
- Legacy compatibility identity: `#dvfnn`.
...
- Automatic release intent resolution must fail closed when a merge commit cannot be mapped to one merged PR on `main`. GitHub GraphQL commit associations are the primary lookup because REST commit associations can be empty for a valid squash merge; REST remains a bounded fallback, while `type:none` is valid only after the PR and its labels have been resolved.
- Resolver failures caused by rate limits are transient only when GitHub identifies them as such. GraphQL error payloads and REST 403 responses therefore require explicit rate-limit signals before using the bounded retry budget; ordinary authorization failures remain fail-closed.
