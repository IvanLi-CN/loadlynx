...
> 当前 owner-facing 名称统一为 `仪表盘`，对应 canonical route 仍为 `/$deviceId/cc`。
> 壳层与一级导航契约以 [`m3n8p-web-top-nav-device-workspace`](../m3n8p-web-top-nav-device-workspace/SPEC.md) 为准；本文只约束仪表盘内部的仪器风格布局与信息层级。

## 背景 / 问题陈述

- 现有 Web 的 `/$deviceId/cc` 页面同时承载监视与控制，但布局偏功能堆叠，缺乏“大屏仪器感”和清晰的视觉层级。
...
- 将完整 USB-PD 查询 / 校验 / Apply 工作流吸收到仪表盘控制区，监视区继续保留 PD 摘要。
...
- `web/src/routes/device-cc.tsx`（仪表盘）布局重构为左监右控。
- 抽取可复用 UI 组件（监视区卡片、控制区模块）。
- 统一视觉风格为“仪器大屏”风格（暗色面板、发光读数、仪表层级）。
- 修正文案与信息层级（模式、输出、故障/保护等）。
- `device-status` 页面保留诊断定位，入口弱化但可达；其中与 PD 相关的入口进入仪表盘 panel，而不是独立一级页面。
...
- 左侧为监视区，右侧为控制区；监视区再明确拆成 `P0 主要信息` 与 `P1 次要信息` 两层。
- `P0 主要信息` 必须包含：
  - 当前模式主读数（由 `CC/CV/CP/CR` 决定主指标）
  - 与设备名称同级的重要读回指标：`Voltage / Current / Power / Resistance`
  - 一张以时间为 X 轴的趋势图，固定展示 `Voltage / Current / Power` 三条线，并用横向虚线标注当前生效的目标值或限制值
- `P1 次要信息` 至少包含：热状态、故障/保护摘要、链路/模拟状态、PD 摘要。
- 右侧控制区必须包含：模式切换、输出开关、设定值（Set）与读回（Readback）并列、保护限制、预设应用，以及完整 USB-PD panel 的入口 / 嵌入态。
...
- 信息层级必须明确为：
  - `P0 主信息`：第一眼判断设备运行是否正常、是否在目标状态的信息。
  - `P1 次要信息`：帮助解释主信息与异常原因的信息。
  - `P2 控制 / 一般信息`：所有会改变设备状态或描述控制上下文的信息。
- 信息层级：主读数 > 次级读数 > 说明文字；主读数字号至少为次级的 1.5×。
- 左监右控固定布局（>=1280px）：监视区宽度 60–65%，控制区 35–40%；两列同高对齐。
- 左侧监视区上半部必须优先让 `设备名 + P0 主信息 + 时间趋势` 形成连续视线，不得被状态小卡或控制说明打断。
- Readback vs Setpoint：必须明确标注（Read/Set），并采用不同色阶与字重；单位始终显示。
- 趋势图的 X 轴语义必须是时间，不得只是抽象 sparkline；当前产品约定固定为最近 30 秒。
- 趋势图同时显示 `Voltage / Current / Power` 三条线；三者共享时间轴，但各自依据本指标的目标值 / 限制值与最近 30 秒读回数据独立求显示域。
- 趋势图内每个指标都必须显示一条当前生效的横向虚线参考线：
  - Voltage：`CV -> Target Voltage`，`CC/CP -> Min Voltage`
  - Current：`CC -> Target Current`，`CV/CP -> Max Current`
  - Power：`CP -> Target Power`，`CC/CV -> Max Power`
- 趋势图不得显示会误导用户的共享数值 Y 轴；量程语义由图例与参考线标签表达。
...
- 以 `device-cc` / `仪表盘` 为唯一单屏入口，重排为左监右控，强调仪器面板感。
...
- `docs/assets/plan-t5x4k-web-instrument-control-ui/instrument-dashboard-mock.png`
