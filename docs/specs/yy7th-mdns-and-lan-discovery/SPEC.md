...
6. 用户勾选要添加的设备，点击“添加到列表”，前端按 `identity.device_id` 合并保存；若已有同一硬件的 USB/devd 或 Web Serial 记录，则只为该设备补充 WiFi/LAN transport，不新增第二台设备；
...
- `transports`: 同一 `identity.device_id` 下可同时保存 `LAN/WiFi`、`USB/devd`、`Web Serial` 等连接方式；扫描、手工添加、USB/devd identity 读取、Web Serial identity 捕获都必须合并到同一设备记录。
