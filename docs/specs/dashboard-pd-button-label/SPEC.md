# 主界面：PD 按钮两行文案规范（Detach / PPS / Fixed）

> 说明：当前主界面这一区域的设计与交互，统一以 `docs/specs/dashboard-extended-voltage-toggle/SPEC.md` 为准；本文保留为早期两行 `PD` 文案设计记录。
...
- 仪表盘内 `PD Panel` 的文案与布局（已由 `docs/specs/usb-pd-pps-and-fixed-settings/SPEC.md`/`docs/specs/pd-settings-touch-value-editor/SPEC.md` / `docs/specs/web-usb-pd-settings/SPEC.md` 管控）。
- Web 端显示（如需同步另开计划）。
...
- 组件契约：`contracts/ui-components.md`
...
- 相关计划：`docs/specs/usb-pd-pps-and-fixed-settings/SPEC.md`、`docs/specs/pd-settings-touch-value-editor/SPEC.md`
