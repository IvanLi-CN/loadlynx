...
- Legacy compatibility identity: `#yff7v`.
...
- Preserve the legacy spec ID `0024` and slug `web-version-github-link` for traceability.
...
- GitHub Pages version provenance was tightened so the visible footer, HTML shell, and `version.json` are all derived from the released Web artifact.
