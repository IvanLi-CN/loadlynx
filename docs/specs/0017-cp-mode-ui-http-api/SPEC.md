...
- Status: 已完成
- Last: 2026-01-18
- Legacy source: `docs/specs/0017-cp-mode-ui-http-api/SPEC.md`
...
- 不实现 Web UI（另起计划：`docs/specs/0018-web-cp-mode-ui/SPEC.md`）。
...
- CP 模式基本语义：
  ...
  - 目标功率可编辑范围：`0 <= target_p_mw <= max_p_mw`（`max_p_mw` 来自当前 preset 的功率上限；默认值见 `docs/specs/0002-cv-mode-presets/SPEC.md`，为 `100_000 mW`；当前固件硬上限参考 `LIMIT_PROFILE_DEFAULT.max_p_mw = 100_000 mW`）；
