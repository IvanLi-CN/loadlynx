...
- `loadlynx-user-operations` now assumes an end-user machine and covers released host-tools installation, USB-first CLI access, HTTP fallback, GitHub Release firmware download, released CLI flash workflows, CLI device memory, and CLI WiFi capability checks without requiring a source checkout.
