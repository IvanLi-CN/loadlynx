...
- Temporary USB/devd candidate IDs are not operation targets. They are valid only inside the `loadlynx device add` USB bind flow, either through interactive TTY selection or non-interactive `--usb-port-path <path>` selection; bind reads USB identity and rejects devices that do not expose a stable `loadlynx-<short-id>` device ID. `status --device` and other direct candidate operations return bind-first errors.
