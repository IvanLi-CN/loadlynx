...
- `loadlynx status` uses the selection chain `--device > nearest ancestor .loadlynx > default_hardware_id > interactive saved-device selection`, and JSON automation receives a structured no-selection error if no remembered device exists. Saved USB operations can recover from a fresh devd device table by scanning, leasing, and confirming `identity.device_id` before issuing the operation.
