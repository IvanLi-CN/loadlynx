...
- 用户版必须要求真实 ESP32-S3 flash 具备 artifact/hash/target evidence、typed phrase、非项目固件风险确认（如适用）和 post-flash identity capture；不能只凭 flash 命令成功宣称设备可用。
...
- 固件：下载 Release 固件 catalog/assets，先 dry-run，再在身份、target、artifact、hash、lease/session、typed phrase 和 post-flash identity capture 清楚时 real flash。
