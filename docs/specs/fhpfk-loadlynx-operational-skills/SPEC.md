...
- 用户版必须要求 CLI 先绑定设备再操作设备。当前实现使用 `loadlynx devices`、`loadlynx device list|add|use|remove`、`loadlynx status` 和 `loadlynx status --device <id>`；同一设备同时存在 USB 与 HTTP transport 时按 `last_transport` 选择。临时 USB/devd candidate ID 只能用于 `loadlynx device add` 的交互绑定流，不能直接用于 status、控制、诊断、烧录或监控。
...
- CLI Hardware Memory:
  ...
  - `loadlynx device add` 在交互 TTY 中扫描并选择 USB 设备；`loadlynx device add --url ...` 通过 HTTP identity 绑定同一设备的 LAN transport。
