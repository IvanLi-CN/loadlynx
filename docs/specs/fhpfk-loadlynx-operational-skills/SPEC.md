...
- Status: 已更新；用户侧硬件操作仅允许 CLI，CLI WiFi 配置与硬件记忆仍是实现门槛，不能在 skill 中伪装成已发布能力。

## 背景 / 问题陈述

LoadLynx 需要两份互补的 agent skill。用户版面向没有源码工程的普通用户机器，必须围绕 GitHub Releases 发布的程序和固件资产组织；开发者版面向源码工程、工具链、发布维护与 HIL 调试，必须先确认或准备 checkout。
...
- 保持两个可安装 skill：
  ...
  - `loadlynx-developer-operations`：开发/维护机器；验证本地工程，必要时 clone canonical repo，再使用源码命令。
- 用户版覆盖 released `loadlynx` CLI 硬件操作、USB/devd 优先连接、HTTP fallback、CLI 硬件记忆能力自检、GitHub Release 固件下载、CLI 烧录，以及 CLI WiFi 配置能力自检。
- 开发者版覆盖源码构建、`just` 命令、devd/CLI 本地运行、release 资产维护、固件、WiFi、校准、reset/monitor、HIL。
...
- 不在本 spec 直接实现固件协议、devd API、CLI WiFi 命令或 CLI 硬件记忆命令；这些属于开发者 skill 引导的后续源码工作。
...
- `.github/workflows/release.yml` 与 `.github/workflows/dev-release.yml` 的发布资产要求。
...
- `loadlynx-devd`：`host-tools` 内的本地 USB CDC bridge 守护程序，提供 localhost devd API。
- `loadlynx`：`host-tools` 内的 released CLI。当前源码可见命令包括 `discover`、`devices`、`status`、`output set`、`usb-port set`、`flash`、`reset`、`monitor`；是否包含 WiFi 命令或用户级硬件记忆命令必须以用户安装版本的 `loadlynx --help` 为准。
...
- 用户版必须提供 host-tools 安装流程：选择 GitHub Release、按平台下载、解压到用户目录、配置用户级 `PATH`、验证 `loadlynx-devd --help` 和 `loadlynx --help`。
- 用户版通过 skill 操作硬件时只支持 `loadlynx` CLI，不得使用 Web UI、raw HTTP 写入、浏览器 local storage 或 Web 控制台作为硬件操作路径。
- 用户版连接顺序必须是 USB/devd 优先，HTTP 其次。HTTP 只在 USB 不可用、用户明确选择 HTTP、或使用已保存 HTTP 设备时作为 fallback。
- 用户版必须要求 CLI 记忆用户保存或连接过的硬件，包括可列出、选择、更新、遗忘曾经连接过的 USB 与 HTTP 设备；同一硬件同时有 USB 与 HTTP 记录时优先 USB。
...
- 硬件记忆：CLI 负责保存/列出/选择/更新/遗忘曾经连接过的 USB 与 HTTP 设备，后续操作优先使用保存的 USB 设备。
...
- CLI Hardware Memory:
  ...
  - CLI 必须支持保存、列出、选择、更新、遗忘曾经连接过的 USB 与 HTTP 设备。
...
- Given 用户请求查找曾经连接过的硬件
  When installed `loadlynx --help` 没有保存/列出/选择硬件的命令
  Then agent 明确报告当前 release 缺少用户侧 CLI 硬件记忆，并升级到开发者实现/发布任务。
...
- skill 不得把未实现的 CLI WiFi 配置、CLI 硬件记忆或缺失的 Release firmware catalog 写成已可用功能。
...
- [x] M2: 用户版写入 released host-tools 安装、USB 优先 / HTTP fallback、GitHub 固件下载、CLI 烧录、CLI WiFi 与硬件记忆能力自检。
...
- [ ] M6: 若要完整支持用户侧硬件查找，先实现并发布 CLI 用户级硬件记忆：保存、列出、选择、更新、遗忘 USB 与 HTTP 设备。

## 风险与开放问题

- 当前 release host-tools 是压缩包中的 CLI/daemon binaries，不是签名桌面安装器。
- 当前源码可见的 `loadlynx` CLI 没有 WiFi 配置命令；用户版只能做能力自检和阻断，不能给出假命令。
- 当前源码可见的 `loadlynx` CLI 没有完整用户级硬件记忆命令；用户版只能要求能力自检和阻断，不能退回 Web UI 记忆。
