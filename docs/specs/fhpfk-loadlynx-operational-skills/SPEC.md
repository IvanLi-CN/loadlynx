...
- 用户版必须要求 CLI 先绑定设备再操作设备。当前实现使用 `loadlynx devices`、`loadlynx device list|add|use|remove`、`loadlynx status` 和 `loadlynx status --device <id>`；同一设备同时存在 USB 与 HTTP transport 时按 `last_transport` 选择。临时 USB/devd candidate ID 只能通过 `loadlynx device add` 的交互选择或 `--usb-port-path <path>` 显式端口绑定流进入设备记忆，不能直接用于 status、控制、诊断、烧录或监控。
...
- WiFi：使用 released `loadlynx wifi show|set|clear` 命令；保护 PSK，不把密钥写入聊天、日志、截图、trace 或 PR。
...
- CLI Hardware Memory:
  ...
  - `loadlynx device add` 在交互 TTY 中扫描并选择 USB 设备；`loadlynx device add --usb-port-path <path>` 非交互绑定指定 USB CDC 端口；`loadlynx device add --url ...` 通过 HTTP identity 绑定同一设备的 LAN transport。
...
- User Workflows:
  ...
  - CLI WiFi 配置：`wifi show|set|clear`，必须先确认 installed CLI 真实存在对应命令。
