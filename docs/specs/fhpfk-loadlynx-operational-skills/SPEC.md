...
- 用户版覆盖 released `loadlynx` CLI 硬件操作、USB/devd 优先连接、HTTP fallback、CLI 硬件记忆、GitHub Release 固件下载、CLI 烧录，以及 CLI WiFi 配置能力自检。
...
- CLI 硬件记忆：`loadlynx hardware ...` 维护的用户级设备清单。默认路径为 macOS `~/Library/Application Support/LoadLynx/devices.json`，Linux `${XDG_CONFIG_HOME:-~/.config}/loadlynx/devices.json`，Windows `%APPDATA%\LoadLynx\devices.json`；`LOADLYNX_HOME` 可覆盖目录。
...
- 用户版必须要求 CLI 记忆用户保存或连接过的硬件，包括可列出、选择、更新、遗忘曾经连接过的 USB 与 HTTP 设备；同一硬件同时有 USB 与 HTTP 记录时优先 USB。当前实现使用 `loadlynx hardware path/list/save/forget`、`loadlynx status --hardware <id>`，并在 `status --device` / `status --url` 成功后自动更新用户级记忆。
...
- 硬件记忆：CLI 负责保存/列出/选择/更新/遗忘曾经连接过的 USB 与 HTTP 设备，后续操作优先使用保存的 USB 设备；该记忆属于用户配置目录，不属于源码 checkout 或 `.esp32-port` 开发守卫。
...
- CLI Hardware Memory:
  ...
  - CLI 支持 `loadlynx hardware path/list/save/forget`、`loadlynx status --hardware <id>`，并在成功 `status --device` / `status --url` 后自动更新曾经连接过的 USB 与 HTTP 设备。
...
- [x] M6: 实现 CLI 用户级硬件记忆：保存、列出、选择、更新、遗忘 USB 与 HTTP 设备，并保存到用户配置目录。
