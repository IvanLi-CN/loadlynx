...
- `loadlynx`：`host-tools` 内的 released CLI。当前源码可见公开 owner-facing 命令包括 `completion`、`devices`、`device`、`status`、`flash`、`cc` / `cv` / `cp`、`pd`、`wifi`、`control`、`preset`；`usb-port set`、`reset`、`monitor` 等维护命令可存在但应隐藏。是否包含具体 WiFi/业务能力必须以用户安装版本的 `loadlynx --help` 为准。
