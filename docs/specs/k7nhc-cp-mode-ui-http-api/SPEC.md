...
- 目标是在 **不依赖 Web UI** 的前提下，让用户可以通过本机屏幕/旋钮/触控与 HTTP API 使用 CP 模式，并为后续 Web UI（见 #ejgp8）提供稳定接口。
...
- 不实现 Web UI（另起计划：`docs/specs/ejgp8-web-cp-mode-ui/SPEC.md`）。
...
- CP 模式基本语义：
  ...
  - 目标功率可编辑范围：`0 <= target_p_mw <= max_p_mw`（`max_p_mw` 来自当前 preset 的功率上限；默认值见 `docs/specs/exkw2-cv-mode-presets/SPEC.md`，为 `100_000 mW`；当前固件硬上限参考 `LIMIT_PROFILE_DEFAULT.max_p_mw = 100_000 mW`）；
...
- Given `output_enabled=true` 且模拟板无故障、链路健康，
  ...
  Then 响应速度满足下方“CP 瞬态响应（内部自测口径，冻结）”中的 `t_10_90/t_90_10 <= 1ms`，且超调/欠调满足同节“峰值偏差”约束。
...
- 为避免“通过 1ms 但存在严重超调/振铃”的情况，本计划对步进的峰值偏差增加约束（同为内部自测口径）。
- 定义：
  - `Δ = |P2 - P1|`（步进幅度）
