...
- Digital firmware stores user WiFi credentials in a dedicated EEPROM blob. Web/devd sends `set_wifi_config` with `wait=false`, returns after the firmware acknowledges the EEPROM write, and relies on follow-up WiFi status polling for connection progress; `clear_wifi_config` invalidates the blob and follows the same storage-acknowledged request / status-poll model. Default development builds do not read repo-root `.env`, and repo-root `.env` must not contain WiFi credentials. Factory WiFi requires explicit `LOADLYNX_ENABLE_FACTORY_WIFI=1` plus command-scoped `LOADLYNX_FACTORY_WIFI_*`. When no user EEPROM blob and no explicit factory fallback are present, runtime WiFi reports `source=none` and stays idle.
...
- CLI business workflows cover WiFi show/set/clear, control get/set, preset list/set/apply, calibration profile/mode/apply/commit/reset, soft-reset and diagnostics export. Control writes require explicit `--enable` or `--disable`. WiFi writes require an independent USB/devd management path; LAN HTTP is not a valid continuation path for `wifi set`, `wifi clear`, or backup restore of `settings.wifi`. CLI output is human-readable by default, with `--json` preserving structured automation output.
- Web Settings exposes devd-backed WiFi status/config and diagnostics export. If the active path is LAN HTTP, WiFi writes must switch to USB/devd or block; LAN HTTP confirmation alone is not a valid safety gate. USB/devd writes proceed as local physical-access operations.
...
- Digital firmware no longer requires or accepts repo-root `.env` WiFi variables for development builds; factory WiFi credentials are compiled only when `LOADLYNX_ENABLE_FACTORY_WIFI=1` and `LOADLYNX_FACTORY_WIFI_*` are explicitly set for that build command.
