...
- Serial owner cleanup is generation-checked so an exiting worker cannot unregister a replacement owner, leases retain the port snapshot needed for cleanup even if the device record is removed, and flash/reset exclusive reservations use a drop guard so cancellation cannot leave a port permanently busy.
