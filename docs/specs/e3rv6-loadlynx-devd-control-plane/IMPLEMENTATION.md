...
- CLI hardware memory uses registry schema 2. The registry key is the stable `identity.device_id`, records may contain both USB and HTTP transport locators, `last_transport` records the selected transport, and `default_hardware_id` powers selector-free `loadlynx status`. Legacy flat USB/HTTP records are migrated when read. ESP32-S3 USB and HTTP identity both use the MAC-derived `loadlynx-<short-id>` key; configured hostnames remain human-facing metadata and are not accepted as hardware IDs.
- `hardware bind usb --candidate ...` is the only user path from a temporary USB/devd candidate into saved hardware. The bind flow creates a short bind-probe lease, reads identity, and rejects firmware whose `identity.device_id` is missing or is not a stable `loadlynx-<short-id>` ID. Bind-probe leases are stored as restricted leases and are rejected by normal operation lease checks. Saved USB operations on a fresh devd retry `device_not_found` or `identity_confirmation_mismatch` by scanning and matching the saved port path before creating an identity-confirming lease; CLI validation treats a missing `identity_device_id` in the lease response as a mismatch whenever a saved stable hardware ID is expected.
...
- Digital firmware keeps the USB identity response compact enough to survive ESP32-S3 USB Serial/JTAG log interleaving. The full matched response carries `identity.device_id=loadlynx-<short-id>` and also emits a compact `stable_identity` object near the firmware metadata so devd can recover the stable hardware ID from fragmented post-transmit frames. Generic `digital-esp32s3` identities remain unbindable and cannot authorize saved control operations.
- Real digital flash has a narrow recovery path for devices already stuck on generic-identity firmware: when the CLI explicitly sends `expected_identity_device_id=digital-esp32s3`, devd may create the preflash lease from a timeout/missing/invalid identity probe and then must prove success with post-flash identity capture. This fallback does not apply to bind, stable saved-hardware identity checks, port-open failures, port conflicts or ordinary control requests.
...
- Current registry v2 verification: `cargo test --manifest-path tools/loadlynx-devd/Cargo.toml --locked` passed and covers hardware bind/default/use command parsing, schema migration, bind-probe lease restriction, missing lease identity rejection, JSON error classification and fresh devd scan-before-lease retry. Digital firmware builds passed for both release defaults and `--no-default-features`; USB identity and calibration profile code do not depend on importing network-only symbols.
