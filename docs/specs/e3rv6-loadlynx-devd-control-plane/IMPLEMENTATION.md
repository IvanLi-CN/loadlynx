...
- Serial owner cleanup is generation-checked so an exiting worker cannot unregister a replacement owner, and flash/reset exclusive reservations use a drop guard so cancellation cannot leave a port permanently busy.
