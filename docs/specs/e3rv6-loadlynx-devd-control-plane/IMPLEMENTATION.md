...
- `hardware bind usb --candidate ...` is the only user path from a temporary USB/devd candidate into saved hardware. The bind flow creates a short bind-probe lease, reads identity, and rejects firmware whose `identity.device_id` is missing or is not a stable `loadlynx-<short-id>` ID. Saved USB operations on a fresh devd retry `device_not_found` or `identity_confirmation_mismatch` by scanning and matching the saved port path before creating an identity-confirming lease.
