...
`mains-aegis` 的 `mains-aegis:tools/mains-aegis-devd` 已验证一个更稳的模式：本地 daemon 作为 Web/App/CLI 的唯一 USB HTTP bridge，负责设备扫描、显式绑定、独占 lease、固件 artifact 校验、烧录、reset、monitor 与 bounded logs。LoadLynx 需要类似能力，但不能直接照搬，因为 LoadLynx 有两个烧录目标、两个硬件选择器、ESP32-S3 到 STM32G431 的 UART 控制链路，以及高风险电子负载输出控制。
...
- MUST: Calibration Commit and Reset success responses require EEPROM read-after-write verification. Commit persists the complete four-curve candidate even when the requested curve already matches RAM after Apply; it may publish RAM only after format, hardware revision, CRC and point-for-point readback verification succeeds. Profile and diagnostics reads expose the resulting persistence status.
...
- Calibration profile reads may use a compact USB response (`cal_profile_v1`) so maximum-size valid calibration curves fit in one JSONL frame. devd must expand that compact response, including persistence status, into the HTTP/Web calibration profile shape before returning it to CLI or Web callers.
