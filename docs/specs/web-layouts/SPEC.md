...
> 当前有效 shell 契约已由 [`web-top-nav-device-workspace`](../web-top-nav-device-workspace/SPEC.md) 接管。本文继续作为布局分层与 pathless layout 抽象的长期约束，不再声明 sidebar / drawer 为当前产品真相。
...
另见：`docs/specs/web-top-nav-device-workspace/SPEC.md`（当前顶部导航 shell / 设备切换 / 系统域契约）。
