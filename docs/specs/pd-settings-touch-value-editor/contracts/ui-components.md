...
- `focused_digit`: `None | Ones | Tenths | Hundredths`（具体集合待 `docs/specs/pd-settings-touch-value-editor/SPEC.md` 冻结）
...
- 步长导致的“显示位不可逐 1 变化”：允许出现仅 `0/2/4/6/8` 或 `0/5` 的情况（具体由 `docs/specs/pd-settings-touch-value-editor/SPEC.md` 的编辑位集合决策决定）。
