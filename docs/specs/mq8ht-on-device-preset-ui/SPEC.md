...
- Preset 数据模型与安全语义基线：`docs/specs/exkw2-cv-mode-presets/SPEC.md`
...
- 用户可见语义（对外缩写）：
  - `UVLO`：欠压（阈值为当前预设的 `min_v_mv`）：
    - **Enable 预检拒绝**（启动前就欠压）：当用户尝试启用 `LOAD` 且 `V_main ≤ min_v_mv`，拒绝启用（播放一次 `UI fail` 失败音），**不进入连续告警**；右下角短暂显示 `UVLO`（闪烁，用于解释无法启用；例如持续 ~3s）。
      ...
      - 若用户希望“先打开 LOAD 再接入被测设备”，可将 `UVLO(min_v_mv)` 设为 `0` 以禁用该预检（见 `docs/specs/trrw7-preset-ui-protection-labels/SPEC.md`）。
...
建议的节奏基线（实现可在 `docs/specs/bd4vh-prompt-tone-manager/SPEC.md` 的 `FaultAlarm` 基础上扩展；需实机试听后微调，但必须保证 primary/secondary 可区分）：
