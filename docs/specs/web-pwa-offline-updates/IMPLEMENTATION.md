...
- Added explicit `LOADLYNX_PWA_MIGRATION=1` build gating for `skipWaiting` and `clientsClaim`; ordinary production builds keep prompt-style activation.
- Made the operator refresh action reload even when no waiting worker is present.
...
- Added production preview smoke coverage for a synthetic pre-guard legacy dashboard shell that still requests `/assets/index-SkMVprsZ.js`.
