...
- 目标是在 **不依赖 Web UI** 的前提下，让用户可以通过本机屏幕/旋钮/触控与 HTTP API 使用 CP 模式，并为后续 Web UI（见 `docs/specs/web-cp-mode-ui/SPEC.md`）提供稳定接口。
...
- 不实现 Web UI（另起计划：`docs/specs/web-cp-mode-ui/SPEC.md`）。
