...
- 在 `docs/specs/cp-mode-ui-http-api/SPEC.md` 提供 CP 模式 + HTTP API 后，需要在 `web/` 增加对应的控制页面，便于在桌面/移动端进行 CP 设定与诊断。
...
- 不在本计划内实现固件侧 CP（见 `docs/specs/cp-mode-ui-http-api/SPEC.md`）。
...
- API client：
  - 增加 CP 相关类型与请求封装（消费 `docs/specs/cp-mode-ui-http-api/SPEC.md` 的 HTTP API 契约）。
- Mock backend：
...
- 不改固件端接口形状；如发现接口不足，必须回到 `docs/specs/cp-mode-ui-http-api/SPEC.md` 的 contracts 增量演进再继续 Web。
...
- 状态解释：
  - 当 CP 受限（例如电流限流导致功率达不到）时，UI 有明确提示（若 `docs/specs/cp-mode-ui-http-api/SPEC.md` 提供可诊断字段则优先使用）。
- 刷新策略：
...
（本计划不新增/修改固件侧 HTTP API；消费的外部契约以 `docs/specs/cp-mode-ui-http-api/SPEC.md` 为准。此处只冻结 Web 内部“路由与组件接口”。）
...
- 以 `docs/specs/cp-mode-ui-http-api/SPEC.md` 的 HTTP API contracts 作为唯一外部依赖口径；Web 不做历史 schema 兼容兜底（接口不够用就先改 contracts）。
...
- 若 `docs/specs/cp-mode-ui-http-api/SPEC.md` 在实现时未提供足够的可诊断字段，Web 侧可能难以解释 “功率达不到” 等状态（需要在 contracts 阶段补齐）。
